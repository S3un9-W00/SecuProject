...
- 3회 커밋: **게임 로그 로드 및 재생(Replay 시스템 - 2단계)** - 저장된 .log 파일을 불러와 프레임 생성 후 웹에서 리플레이 재생

- 4회 커밋: **AI 강화(왔던길 기억. 막다른 길 기억)** - 
AI BOT이 자기가 왔던길을 계속 돌거나, 막다른 길에 계속 멈추는걸 방지하기 위해 자기가 갔던 길은 기억을 하는 봇으로 강화
...
├── SecuprojectApplication.java  # Spring Boot 메인 클래스
├── Maze_one.java          # 1단계: 콘솔 기반 미로 게임
├── Maze_two.java          # 2단계: 개선된 미로 클래스 (안개, 아이템, 함정)
├── Maze_swing.java        # 3단계: Swing GUI 게임
                               ├── Enemy.java             # 오른손 법칙 AI 로봇
                               ├── EnemyMemory.java       # AI 메모리 (방문 기록, 루프 방지)
                               ├── controller/
                               │   └── GameController.java # Spring REST 컨트롤러
├── Service/
...
│   └── GameLogger.java
├── replay/                # 리플레이 시스템
│   ├── ReplayGenerator.java
│   └── ReplayFrame.java
├── ui/
│   └── GameUI.java       # Swing UI
└── util/                  # 유틸리티
    ├── MazeGenerator.java  # 미로 생성 (Standalone)
    ├── MazeValidator.java  # 미로 검증
...
**개요**: Maze_one을 개선하여 재사용 가능한 클래스로 만든 버전입니다. 안개, 아이템, 함정(스프링 연동)을 포함합니다.

**주요 개선사항**:
- `MoveResult` 클래스로 이동 결과 구조화
- `showMaze()` 메서드가 String 반환 (콘솔 출력 분리)
- 횃불(시야 5×5, 10초), 망치(벽 1회 파괴), 함정(3초 이동 불가) 추가
- 안개 시스템 개선 (기본 3×3, 아이템 시 5×5)

**개발 시 고려사항**:
- 객체 지향 설계 원칙 적용
- 메서드 분리로 재사용성 향상
- 상태 관리 개선 (torchEffect/hammer/trap 타이머)

**주요 로직**:
```java
// 미로 표시 (문자열 반환)
public String showMaze() {
    // 횃불 효과 시간 체크 (10초 후 원복)
    // 플레이어 주변 시야 계산
    // 미로 상태를 문자열로 변환
}
...
### 개요
게임 진행 중 모든 이벤트를 로그로 저장하고, 게임 종료 후 로그 파일을 로드하여 조작 과정을 다시 재생합니다
...
### 재생 알고리즘 (과제 5-2: 로드/재생)
핵심 아이디어는 **초기 상태 + 이벤트 순서 재적용**입니다.

- **1) 로그 로드**: GameLogger.loadLog(file)로 GameLog 역직렬화
- **2) 초기 상태 복원**: initialMaze로 Maze_two를 새로 만들고 시작 좌표를 기준으로 첫 화면(START 프레임) 생성
- **3) 이벤트 정렬**: events를 timestamp 오름차순으로 정렬
- **4) 이벤트 적용 -> 프레임 생성**
  - MOVE면 maze.move(direction)`을 호출해 동일한 이동/아이템/함정 로직을 다시 적용
  - AI_MOVE면 이벤트에 기록된 적 좌표를 반영
  - 매 이벤트마다 maze.showMaze(enemyX, enemyY)로 현재 화면 문자열을 만들고 ReplayFrame으로 저장
- **5) 시간차로 재생**: 프론트에서 offsetMillis 차이만큼 기다리며 프레임을 순서대로 출력(속도 배수로 조절)

### 웹에서 리플레이 보는 법
- 게임 완료 팝업(또는 게임 화면)에서 **리플레이 보기** 버튼 클릭 -> /replay
- 파일 선택 후 **불러오기 -> 재생/일시정지/한 프레임** 가능
